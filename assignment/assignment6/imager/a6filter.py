...
class Filter(a6editor.Editor):
    ...
    def jail(self):
        ...
        * Put n 4-pixel vertical bars inside, where n is 
        ...
        # total_space = current.getWidth()-8

        # if n!=0:
        #     spaceBtwTwoBars= total_space / (n + 1)
        #     start= 1
        
        #     while start < n + 1:
        #         col = round(start * spaceBtwTwoBars)
        #         self._drawVBar(col + 2, (255, 0, 0))
        #         start=start+1
    ...
    def vignette(self):
        ...
        for i in range(current.getHeight()):
            for j in range(current.getWidth()):
                pixel=current.getPixel(i,j)
                d=((current.getWidth()/2-i)**2+(current.getHeight()/2 - j)**2)**0.5
                hfd=((current.getWidth()/2)**2+(current.getHeight()/2)**2)**0.5
                multiplier=1-(d/hfd)**2
                red=int(pixel[0]*multiplier)
                green=int(pixel[1]*multiplier)
                blue=int(pixel[2]*multiplier)
                current.setPixel(i, j, (red, green, blue))
